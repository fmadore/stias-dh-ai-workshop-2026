...
**The One Gold Moment Rule.** Brass appears as a solid fill exactly once on any given page. It has two sanctioned forms, and they are mutually exclusive per page: the `.btn-accent` (the hero's primary action, the call for papers' download) and `.day-pill.is-today` (the programme, during the workshop only). Everywhere else brass is a 2px rule, a hairline border, or a 6–10% wash. Its rarity is what makes it read as gold rather than as yellow. Note the honest exception pressure: `/programme` also carries 25 two-pixel brass paper rules, one per paper, which is a great many hairlines for a rule whose stated reason is rarity. It is recorded rather than resolved; if it is ever revisited, the answer is a quieter treatment for that rule, not a licence for a second solid fill.
