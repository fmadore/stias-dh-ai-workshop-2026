...
The one rich-text surface on the site: `marked` output rendered at build time into `.prose`. Paragraph rhythm is `--space-stack` via `.prose > * + *`; `strong` goes to 600 in strong ink, `em` to italic (Outfit has no true italic, so nine emphases render as a synthesised oblique — inspected and accepted, because the serif alternative reads worse here); links take the inline-link treatment with a 30% underline that goes solid on hover. An author's own section heading — a paragraph whose entire content is one bold run, which is the only syntax an abstract author had to hand — is tagged `.prose-subhead` at render time, because CSS cannot select "a paragraph that is _only_ a bold run" without also catching one that merely opens with a bold phrase. It sets at the card-title step in the display serif, with 2.25rem above against 0.5rem below, so the interval says which text the heading belongs to. It is an `<h3>`, which required the page to grow an `<h2>` of its own first: the article now opens with an "Abstract" heading in the page's locale, outside the `lang` the abstract carries, so h1 → h2 → h3 holds and five headings that were styled as headings but invisible to the document outline are in it. Without that h2 an `<h3>` here would have filed under the preceding "Presented by", which is not what it belongs to — the structure and the styling had to land together.

The `lang` that article carries is resolved, not read off the paper. Nearly every abstract is a single string in whichever language its author wrote it, and an English reader and a French one are shown the same words — which is why the "Abstract" heading above it is localized and the article below it is not. One author supplied both halves, so `Presentation.abstract` also takes a `LocalizedString`; that kind follows the reader, and `lang` follows with it. It cannot come from `presentation.language`, which describes the _talk_: translating an abstract does not change the language the paper is given in, so the corner mark on the card and the language filter go on saying EN while the prose beside them is French. `resolveAbstract` returns the text and its `lang` together for exactly that reason. Search is the mirror case — `abstractVariants` feeds _both_ halves into the haystack, so a French word finds a bilingual paper from the English index too.
...
**The Resting Affordance Rule.** A link that carries no colour of its own carries a resting underline: `color-mix` at **65% of `primary-600`** in light and **60% of `primary-300`** in dark, 1px thick at a 0.2em offset, going solid `currentColor` on hover and focus. Those percentages are measured, not chosen — 35% light and 40% dark came out at 1.78:1 and 2.18:1, a resting cue you cannot resolve on a phone, which is the original defect wearing a fix's clothes. 65/60 clear 3:1 (measured 3.26:1 and 3.32:1 on the card, 3.20:1 and 3.55:1 under the home page's serif numerals, where the rule steps up to 2px because 1px under a 48px numeral reads as an artefact). **Hover is not an affordance.** 67 of the programme's 91 links had `color: inherit`, no underline, and a colour change on hover alone — no resting cue at all on the page most likely to be read on a phone in a conference room. A link that carries its own glyph (an external-link mark) may opt out; nothing else may. The rule now holds everywhere: `PaperCard`'s title and the paper page's author name were the last two holdouts (P3-20) and carry it at 1.5px, because at the card-title and 18px display steps a 1px rule reads as a hairline artefact rather than an underline.

**The Inert-State Rule.** Before replacing an interactive state, check that it was ever working. The author link declared `color` on the anchor while both its child spans set their own (`text-strong`, `text-muted`), so the hover reached neither and the element's only interactive feedback did nothing at all — a defect identical in the source and identical on screen to one that works, and one no linter, type-checker or axe run reports. Where a link wraps more than one line of differently-coloured text, put the state on the part that is the link's subject, and put the underline there too: ruling a second, muted line reads as two links rather than one. The general form: a declaration on an ancestor that every descendant overrides is not a state, it is a comment.
