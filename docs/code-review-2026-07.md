...
| `marked` is bundled into client chunks although abstracts are prerendered; `abstract.ts` also drags `marked` into the papers _list_ page, which only needs `abstractToPlainText`                          | `utils/abstract.ts:1`, `papers/[slug]/+page.svelte:15`                     | Render markdown in `load()` (build time); split plain-text helpers into a marked-free module                                                                                                         |
