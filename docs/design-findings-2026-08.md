...
### Phase 5 — the document pass

**Run 24 August 2026.** `DESIGN.md` was written on 18 August and nine passes landed after it, so this was a refresh against the shipped tree rather than a first extraction. It is a documentation pass, so it closes nothing; what it produced is an accurate spec, three corrections to claims the file was still making, and one finding.

**What the file had stopped being true about.** Its **Four Containers Rule** named `container-prose`, which `distill` deleted on 21 August for having no consumers — the rule is **Three Containers** now, and it says explicitly that the three inner copy blocks which set their own width inside a container are a different thing and are fine. Its **Anchor Clearance Rule** still pointed at the `scroll-mt-28` literal as "the one offset still not derived", which `harden` replaced with `--day-bar-height`; between them the two height tokens now have **seven consumers**, counted. And its motion promise ("fully surrendered under `prefers-reduced-motion`") described a register of named selectors that `animate` replaced with a universal floor after measuring **twelve transitions surviving it on `/programme` alone**.

**Five rules are new, and each one records a defect the site shipped rather than a principle someone liked.** **The Resting Affordance Rule** (hover is not an affordance; the measured 65% / 60% mixes, and why 35% / 40% was the same defect wearing a fix's clothes). **The Target Floor Rule** (two tiers — 44px for actions and navigation, 24px for an inline affordance in a text row — both declared as `min-height`, because padding does not hold a floor). **The Motion Floor Rule** (a floor, not a register; why the `!important` is there; why animations stay named). **The Contrast-On-Surface Rule** (a role with two backgrounds takes the value that clears the harder one — P1-9's lesson, promoted from an observation to an invariant, together with its corollary that the sweep runs in both colour schemes). **The Min-Height-Is-A-Floor Rule** (a declared constant describing a rendered box must be unable to disagree with it — the day bar at 320px).

**Two things the pass had to correct in itself rather than inherit.** The typography section's claim of a clean five-step small scale did not survive being counted: it is **six values**, and 14px carries two names in live use — `--text-ui-sm` at 3 call sites against Tailwind's `text-sm` at 33, the most-used size utility on the site. And the **One Gold Moment Rule** now names both of its sanctioned forms (`.btn-accent`, and `.day-pill.is-today` during the workshop) and records the pressure on it honestly: `/programme` also carries **25 two-pixel brass paper rules**, which is a great many hairlines for a rule whose stated reason is rarity. That is **P3-34**, left standing rather than quietly written out of the rule it strains.

**The sidecar is now derived, not retyped.** `.impeccable/design.json`'s narrative — north star, overview, key characteristics, all 21 named rules with their sections, and both bullet lists verbatim — is extracted from `DESIGN.md` by script, so the two cannot drift the way the markdown drifted from the code. It gained the **day pill** and the **resting link** as primitives (12 components), five colour-meta entries, and a corrected segmented control: its snippet was still advertising the `aria-pressed` semantics `clarify` removed in favour of `role="radiogroup"`.

**Found in passing and fixed.** The `--day-bar-height` comment still claimed "the bar consumes it as min-height so the number cannot quietly stop being true" — which `adapt` had replaced with `flex-nowrap` for precisely the reason the new rule states. A stale assertion inside the token the rule is about is worse than no comment, so it now says what actually holds the number true.

- **P3-39 · A dead selector in the print stylesheet.** `@media print` styles `.abstract a[href^='http']::after` (`app.css:1164`) to append URLs after external links. No element has carried `.abstract` since the paper page moved to the shared `.prose` class, whose sibling rule on the same line already does the job. Inert — nothing prints wrong — but it is one of the three selectors in a block that only ever runs where nobody looks. → `distill`

---

### After the roadmap — three items closed on request

**Run 24 August 2026.** Two open findings and one of the three decisions that were waiting on the user, done in one pass at their request. Full gate: 20 unit tests, 34 e2e (two new), 0 axe violations across both colour schemes.

- ~~**P3-25 · An abstract's own section headings are absent from the document outline.**~~ **Closed.** The blocker was never the markup — it was that promoting those five `**bold**` paragraphs to `<h3>` would have filed them under the preceding "Presented by", so the fix needed an "Abstract" heading first, and that is new copy in two locales. `paper_abstract` is "Abstract" / "Résumé", set at the eyebrow role like the two `<h2>`s it sits between, and deliberately **outside** the `lang` the abstract carries: the heading is UI copy in the page's locale, the abstract stays in its language of delivery. `renderAbstract` emits `<h3 class="prose-subhead">` now. Verified against the build in both locales: the outline reads h1 → h2 Presented by → h2 Abstract/Résumé → h3 ×5 → h2 Also in this session, one h1, no level skipped. The styling survived the tag change untouched — 20.93px Instrument Serif at 400, leading 1.25, tracking −0.012em, the bold run still 400 — because `.prose .prose-subhead` sits in `@layer components` and outranks the `h3` element default in `@layer base`, which is the layering rule doing exactly what it was built for. Intervals unchanged at 36px above a subhead against 20px between plain blocks.
- ~~**P3-20 · The resting link affordance stops at the programme.**~~ **Closed**, and it was hiding a second defect. `PaperCard`'s title and the paper page's author name now carry the underline at the measured mixes — **3.26:1 light / 3.32:1 dark** on the raised card, **3.20:1 / 3.55:1** on the page — at **1.5px** rather than 1px, because at the card-title step (20.9px) and the author name's 18px display serif a 1px rule reads as a hairline artefact. The underline goes on the author's _name_, not the anchor: the anchor also wraps a 14px muted affiliation line, and ruling both reads as two links.
- **The author link's hover had never worked at all** — recorded here because it is a class of defect, not a one-off. `.author-link:hover` set `color`, while both child spans set their own (`text-strong`, `text-muted`), so the change reached neither and the element's only interactive feedback did nothing. It looks correct in the source and identical on screen, and no tool reports it. The states live on the name now. **Lesson for the next pass: when replacing a hover-only affordance, check the hover was ever working — a dead declaration and a live one are indistinguishable until you measure.**
- **P2-18 · `/venue` has no getting-there facts.** **Substantially closed.** The finding existed because no arrival facts existed in the repository and PRODUCT.md forbids inventing them; the user supplied them: Point Sud has booked flights for all participants, everyone lands at Cape Town International, and transport to Stellenbosch will be organised with details to follow. It renders as a `.callout` under the two logistics lists — the same treatment the programme's "preliminary" notice uses, which is the site's idiom for information that is true now and not yet complete, and the right one here because half this statement is settled and half is not. No overflow at 320px in French, where it sets 216px of measure inside a 288px card. What remains is the detail the note itself promises, which is a content update rather than a design finding.

**A verification note worth carrying forward.** Measuring a theme-dependent custom property by toggling `.dark` in the same synchronous task returns the _previous_ theme's value: the first light-mode reading of the author underline came back 1.64:1 and looked like a real AA failure. It was an artefact. Reload with the theme set — `localStorage.setItem('theme', …)` then `location.reload()` — and the same element measures 3.20:1. Any contrast figure taken from a class toggle should be re-taken after a reload before it is believed, or acted on.
